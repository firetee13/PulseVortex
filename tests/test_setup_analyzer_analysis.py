...
class SetupAnalyzerAnalysisTests(unittest.TestCase):
    ...
    def create_series_data(self, symbol='EURUSD', **overrides):
        ...
        last_snapshot = sa.Snapshot(
            ts=self.sample_snapshot.ts,
            row=canonical_row_data
        )
    ...
    def test_analyze_sl_too_close_to_spread(self, mock_quiet):
        ...
        with patch.dict('monitor.cli.setup_analyzer.os.environ', {'TIMELAPSE_SPREAD_MULT': '10'}):
            results, reasons = sa.analyze(series, 1.0, datetime.now(UTC), debug=False)

            # This might be filtered due to spread distance check
            if not results:
                self.assertTrue(any('sl_too_close_to_spread' in reason for reason in reasons))
    ...
    def test_analyze_sorting_by_score_and_rrr(self, mock_quiet):
        ...
        series1 = self.create_series_data(**{'Strength 1H': 3.0, 'Strength 4H': 4.0, 'Strength 1D': 5.0})
        ...
        results, reasons = sa.analyze(combined_series, 1.0, datetime.now(UTC), debug=False)
...
class SetupAnalyzerDatabaseTests(unittest.TestCase):
    ...
    def test__ensure_proximity_bin_schema_already_exists(self, mock_conn, mock_sqlite):
        ...
        mock_cur.fetchall.return_value = [(0, 'id', 'INTEGER', 0, None, 1), (1, 'proximity_bin', 'TEXT', 0, None, 0)]

        result = sa._ensure_proximity_bin_schema(mock_cur, "test_table")

        self.assertTrue(result)
        # Should not execute ALTER TABLE
        alter_calls = [call for call in mock_cur.execute.call_args_list if 'ALTER TABLE' in str(call)]
    ...
    def test__backfill_missing_proximity_bins(self, mock_connect, mock_sqlite, mock_ensure):
        mock_cur = MagicMock()
        mock_conn = MagicMock()
    ...
    def test__filter_recent_duplicates_no_open_setups(self, mock_db_path, mock_sqlite, mock_connect, mock_backfill, mock_ensure):
    ...
    @patch('monitor.cli.setup_analyzer.sqlite3')
    @patch('monitor.cli.setup_analyzer.default_db_path')
    def test__filter_recent_duplicates_with_open_setups(self, mock_db_path, mock_sqlite, mock_connect, mock_backfill, mock_ensure):
        mock_db_path.return_value = "/test/path"
        mock_conn = MagicMock()
        mock_connect.return_value = mock_conn
        mock_cur = MagicMock()
        ...
        results = [
            {'symbol': 'EURUSD', 'direction': 'Buy', 'proximity_to_sl': 0.45},  # Same bin (0.4-0.5)
            {'symbol': 'GBPUSD', 'direction': 'Sell', 'proximity_to_sl': 0.3},  # Different
        ]
