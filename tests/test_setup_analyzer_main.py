...
class SetupAnalyzerMainFunctionTests(unittest.TestCase):
    ...
    @patch('sys.argv', ['script.py', '--symbols', 'EURUSD,BTCUSD', '--min-rrr', '2.0', '--top', '5', '--brief', '--watch', '--interval', '2.0', '--debug', '--exclude', 'GLMUSD'])
    ...
    def test_process_once_with_exclude_set(self, mock_insert, mock_analyze, mock_read):
        ...
        exclude_set = {'EURUSD'}
        sa.process_once(['EURUSD'], 1.0, None, False, debug=False, exclude_set=exclude_set)

        # Should filter out EURUSD before analysis
        mock_analyze.assert_called_once_with({}, min_rrr=1.0, as_of_ts=unittest.mock.ANY, debug=False)
    ...
    def test_watch_loop(self, mock_sleep, mock_process):
        ...
        sa.watch_loop(['EURUSD'], 1.0, None, False, False, debug=False, exclude_set=None)
    ...
    def test_watch_loop_with_exclude_set(self, mock_sleep, mock_process):
        ...
        sa.watch_loop(['EURUSD'], 1.0, None, False, True, debug=True, exclude_set=exclude_set)
