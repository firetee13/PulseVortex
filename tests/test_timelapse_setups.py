...
class FakeMT5:
    ...
    def __init__(self, tick_time: datetime):
        ...
        self.copy_ticks_history = [
            {
                "flags": self.TICK_FLAG_BID | self.TICK_FLAG_ASK,
                "bid": 1.2340,
                "ask": 1.2344,
            }
        ]
    ...
    def copy_rates_range(
        self, sym: str, timeframe: int, start: datetime, end: datetime
    ):
        return list(self.rates_return.get(timeframe, []))
...
class HelperFunctionsTests(unittest.TestCase):
    def test_infer_decimals_from_price(self):
        ...
        self.assertEqual(
            tls._infer_decimals_from_price(float("nan")), 5
        )  # Exception fallback
    ...
    def test_sell_uses_ask_for_sl_distance(self):
        ...
        last = tls.Snapshot(
            ...
            row={
                ...
                tls.canonicalize_key(
                    "Strength 1W"
                ): -0.1,  # ensure overall Sell (>=2 negatives)
                ...
                tls.canonicalize_key("Last Tick UTC"): now.strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            },
        )
    ...
    def test_analyze_time_filter(self):
        ...
        last = tls.Snapshot(
            ...
            row={
                ...
                tls.canonicalize_key(
                    "Strength 1W"
                ): 0.1,  # ensure overall Buy (>=2 positives)
                tls.canonicalize_key("D1 Close"): 1.05,
                tls.canonicalize_key("D1 High"): 1.15,
                tls.canonicalize_key("D1 Low"): 0.95,
                tls.canonicalize_key("Recent Tick"): 1,
                tls.canonicalize_key("Last Tick UTC"): blocked_time.strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            },
        )
        series = {sym: [first, last]}
        results, reasons = tls.analyze(
            series, min_rrr=1.0, as_of_ts=blocked_time, debug=False
        )
        self.assertEqual(results, [])
...
class ProcessOnceTests(unittest.TestCase):
    ...
    def test_process_once_no_symbols(self):
        ...
        symbols = []
        with patch.object(
            tls, "read_series_mt5", return_value=({}, None, datetime.now(UTC))
        ):
            with patch.object(tls, "analyze", return_value=([], {})):
                tls.process_once(
                    symbols=symbols, min_rrr=1.0, top=None, brief=True, debug=False
                )
        ...
        with patch("time.sleep", side_effect=[None, KeyboardInterrupt()]):
            with patch.object(tls, "process_once") as mock_process:
                with patch("builtins.print") as mock_print:
                    ...
                    self.assertTrue(
                        any(
                            "Will filter out symbols" in call[0][0]
                            for call in mock_print.call_args_list
                        )
                    )
