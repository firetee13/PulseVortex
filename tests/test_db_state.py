...
def test_backfill_handles_missing_setups_table():
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (1, "EURUSD", "buy", 1.0, 1.2, "TP", 1.1, hit_time.strftime("%Y-%m-%d %H:%M:%S")),
    )
    ...
    assert hit_time_utc3 == (hit_time + timedelta(hours=3)).strftime("%Y-%m-%d %H:%M:%S")
...
def test_load_setups_since_hours_filters_rows():
    ...
    rows = load_setups_sqlite(conn, "timelapse_setups", since_hours=2, ids=None, symbols=None)
...
def test_record_hit_uses_precious_metals_digits(monkeypatch):
    ...
    hit = Hit(kind="TP", time_utc=datetime(2024, 1, 1, 1, 0, tzinfo=UTC), price=1951.2345)

    record_hit_sqlite(conn, setup, hit, dry_run=False, verbose=False)

    cur = conn.cursor()
    cur.execute(
        "SELECT sl, tp, hit_price FROM timelapse_hits WHERE setup_id = ?", (9,)
    )
