...
class BarCrossesPriceGapTests(unittest.TestCase):
    ...
    def test_buy_gap_up_above_tp_counts_as_hit(self) -> None:
        bar = self.bar.__class__(
            start_utc=self.bar.start_utc,
            end_utc=self.bar.end_utc,
            low=11970.0,
            high=11980.0,
        )
...
class EvaluateSetupQuietHoursTests(unittest.TestCase):
    ...
    def test_evaluate_setup_skips_quiet_hours_ranges(self) -> None:
        ...
        expected_ranges = [
            (
                datetime(2025, 9, 28, 20, 44, tzinfo=UTC),
                datetime(2025, 9, 28, 20, 45, tzinfo=UTC),
            ),
            (
                datetime(2025, 9, 28, 21, 59, tzinfo=UTC),
                datetime(2025, 9, 28, 22, 0, tzinfo=UTC),
            ),
        ]
